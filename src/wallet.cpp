        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                if (!wtx.IsSpent(txin.prevout.n) && (IsMine(wtx.vout[txin.prevout.n]) || IsMineForMintingOnly(wtx.vout[txin.prevout.n])
                                                     || IsMineForMultiSig(wtx.vout[txin.prevout.n])))
                {
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fFindBlock)
{
    uint256 hash = tx.GetHash();
    {
        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx) || IsMineForMintingOnly(tx) || IsMineForMultiSig(tx))
        {
    ...
    int64 nTotal = 0;
    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (!pcoin->IsFinal() || !pcoin->IsConfirmed() || !pcoin->IsFrozen())
                continue;
            int64 nCredit = pcoin->GetAvailableCredit();
            map<unsigned int,int64>::iterator mi = frozenRet.find(pcoin->nLockTime);
            if (mi != frozenRet.end())
            {
                (*mi).second += nCredit;
            }
            ...
            {
                frozenRet.insert(make_pair(pcoin->nLockTime,nCredit));
            }
...
int64 CWallet::GetFrozenBalance(const CTxDestination &destAddress,map<unsigned int,int64>& frozenRet) const
{
    int64 nTotal = 0;
    vector<COutput> vecOutputs;
    AvailableCoins((unsigned int)GetAdjustedTime(), vecOutputs, true, false, true);
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        {
            int64 nCredit = pcoin->vout[out.i].nValue;
            map<unsigned int,int64>::iterator mi = frozenRet.find(pcoin->nLockTime);
            if (mi != frozenRet.end())
            {
                (*mi).second += nCredit;
...
// lomocoin: total coins staked (non-spendable until maturity)
...
// lomocoin: total coins staked (non-spendable until maturity)
int64 CWallet::GetStake(const CTxDestination &destAddress) const
{
    int64 nTotal = 0;
    LOCK(cs_wallet);
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() > 0 && pcoin->GetDepthInMainChain() > 0)
        {
            ...
            for (int i = 0;i < pcoin->vout.size();i++)
                if (ExtractDestination(pcoin->vout[i].scriptPubKey, address) && address == destAddress)
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // lomocoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // lomocoin: not avatar mode
                    {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    int64 nBalance = GetBalance() + GetFrozenBalance(frozenCoins);
    ...
    {
        if (!SelectMintingCoins(nBalance - nReserveBalance, txNew.nTime, setCoins, nValueIn))
            return false;
    }
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
            {
                ...
                if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH
                     && subType != TX_COLDMINTING && subType != TX_MULTISIGCOLDMINTING)
                {
                    if (fDebug && GetBoolArg("-printcoinstake"))
                ...
                fMintingOnly = (subType == TX_COLDMINTING || subType == TX_MULTISIGCOLDMINTING);
                ...
                txNew.nLockTime = pcoin.first->nLockTime;
                nCredit += pcoin.first->vout[pcoin.second].nValue;
                vwtxPrev.push_back(pcoin.first);
                txNew.vout.push_back(CTxOut(0, scriptPubKeyOut));
                if (fSplitStake && block.GetBlockTime() + nStakeSplitAge > txNew.nTime)
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        // Only add coins of the same key/address as kernel and same nLockTime
        if (txNew.vout.size() == 2 && ((pcoin.first->vout[pcoin.second].scriptPubKey == scriptPubKeyKernel || pcoin.first->vout[pcoin.second].scriptPubKey == txNew.vout[1].scriptPubKey) && pcoin.first->nLockTime == txNew.nLockTime)
    ...
    // Calculate coin age reward
    {
        ...
        if (!fTestNet && txNew.nTime < 1477929600) // 2016.11.1
        {
            nCredit += GetProofOfStakeReward(nCoinAge,0);
        }
        else
        {
            nCredit += GetProofOfStakeReward(nCoinAge,nCredit);
        }
    }
...
// lomocoin: check 'spent' consistency between wallet and txindex
// lomocoin: fix wallet spent state according to txindex
...
// lomocoin: disable transaction (only for coinstake)
void CWallet::DisableTransaction(const CTransaction &tx)
{
    if (!tx.IsCoinStake() || !(IsFromMe(tx) || IsMineForMintingOnly(tx) || IsMineForMultiSig(tx)))
        ...
        if (mi != mapWallet.end())
        {
            ...
            if (txin.prevout.n < prev.vout.size()
                && (IsMine(prev.vout[txin.prevout.n])
                     || IsMineForMintingOnly(prev.vout[txin.prevout.n])
