...
// lomocoin: get network Gh/s estimate
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("transactions",  (int)pwalletMain->mapWallet.size()));
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <lomocoinaddress> <amount> [comment] [comment-to]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <tolomocoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid lomocoin address:")+s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // lomocoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; lomocoin server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "LoMoCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "LoMoCoin is downloading blocks...");

    if (pindexBest->nHeight >= LAST_POW_BLOCK)
        throw JSONRPCError(-1, "No more PoW blocks");
...
// lomocoin: get information of sync-checkpoint
...
// lomocoin: reserve balance from being staked for network protection
...
// lomocoin: check wallet integrity
...
// lomocoin: make a public-private key pair
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: lomocoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use lomocoind";
